...
// Tipe Data
type StockItem = { name: string; warning_limit: number | null; stock: number; };
type Product = StockItem & { id: number; sku: string; category_id: number | null; sort_order: number; };
type Component = StockItem & { id: number; unit: string; usage_type: string; };
...
type ComponentTab = { id: number; type_key: string; display_name: string; };

// Fungsi getStockRowClass disederhanakan untuk konsistensi
const getStockRowClass = (item: Product | Component): string => {
  if (item.stock === 0) return 'stock-danger';
  if (item.warning_limit !== null && item.stock <= item.warning_limit) return 'stock-warning';
  return '';
};
...
export default function App({ user }: AppProps) {
  ...
  const [componentTabs, setComponentTabs] = useState<ComponentTab[]>([]);
  const [activeComponentTabKey, setActiveComponentTabKey] = useState<string>('');
  ...
  const fetchData = async () => {
    try {
      const [compRes, prodRes, catRes, logRes, tabRes] = await Promise.all([
        ...
        supabase.from('component_tabs').select('*').order('sort_order')
      ]);
      
      const errors: PostgrestError[] = [compRes.error, prodRes.error, catRes.error, logRes.error, tabRes.error].filter((e): e is PostgrestError => e !== null);
      ...
      setComponentTabs(tabRes.data || []);

      if ((tabRes.data?.length ?? 0) > 0 && activeComponentTabKey === '') {
        setActiveComponentTabKey(tabRes.data![0].type_key);
      }
  ...
  if (error) {
    return (
      <div className="fullscreen-centered-container error-layout">
        ...
        <button className="reload-button" onClick={() => window.location.reload()}>Muat Ulang Halaman</button>
      </div>
    );
  }
            ...
            <tbody>
              {categories.map(category => (<>
                ...
                {products.filter(p => p.category_id === category.id).map(p => (
                  <tr key={p.id} className={getStockRowClass(p)} onTouchStart={() => {}}>
      ...
      <div className="section-container">
        ...
        <div className="tab-buttons">
          {componentTabs.map(tab => (
            <button 
              key={tab.id}
              className={activeComponentTabKey === tab.type_key ? 'active' : ''}
              onClick={() => setActiveComponentTabKey(tab.type_key)}
            >
              {tab.display_name}
            </button>
          ))}
